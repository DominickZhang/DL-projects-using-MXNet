...
#ZJN: Add test minist
#mnist_train supports indexing while train_data does not
def load_data_mnist_full(resize=None, root="~/.mxnet/datasets/mnist"):
    """download the fashion mnist dataest and then load into memory"""
    def transform_mnist(data, label):
        # Transform a batch of examples.
        if resize:
...
#ZJN: Add a new function
def show_images_1D(images):
    n = images.shape[0]
    _, figs = plt.subplots(1, n, figsize=(5, 5))
 #   print(figs)
    for i in range(n):
        figs[i].imshow(images[i].reshape((28,28)).asnumpy(), cmap='Greys')
...
#ZJN: Add a new function
def show_images_2D(imgs, nrows, ncols, figsize=None):
    """plot a list of images"""
    if not figsize:
        figsize = (ncols, nrows)
#    figs = plt.figure()
    ...
    for i in range(nrows):
        for j in range(ncols):
            figs[i, j].imshow(imgs[i*ncols+j].reshape((28,28)).asnumpy(), cmap='Greys')
            figs[i, j].axes.get_xaxis().set_visible(False)
            figs[i, j].axes.get_yaxis().set_visible(False)
    plt.show()
